...
void Shader::loadMatrix(const char* location, const glm::mat4& matrix, bool trans) const {
	glUniformMatrix4fv(getUnifromLocation(location), 1, trans, value_ptr(matrix));
}
...
void Shader::loadMatrixArray(const char* location, const std::vector<glm::mat4> matrixArray, const unsigned short count, bool trans) const {
	glUniformMatrix4fv(getUnifromLocation(location), count, trans, value_ptr(matrixArray[0]));
}

void Shader::loadVector(const char* location, glm::vec4 vector) const {
	glUniform4fv(getUnifromLocation(location), 1, value_ptr(vector));
}

void Shader::loadVector(const char* location, glm::vec3 vector) const {
	glUniform3fv(getUnifromLocation(location), 1, value_ptr(vector));
}

void Shader::loadVector(const char* location, glm::vec2 vector) const {
	glUniform2fv(getUnifromLocation(location), 1, value_ptr(vector));
}
