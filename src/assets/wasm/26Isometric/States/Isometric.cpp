...
Isometric::Isometric(StateMachine& machine) : State(machine, States::ISOMETRIC), m_bulletStore(), m_enemySpawner(120.0f * 0.0044f, m_player) {
	...
	SoundDevice::Init();
	m_fire.init<OpenALEffect>();

	m_ding.init<OpenALEffect>();
	m_ding.get<OpenALEffect>()->setVolume(0.5f);
...
std::vector<WGPUBindGroupLayout> Isometric::OnBindGroupLayoutsWiggly() {
	...
	bindingLayoutEntries[4].buffer.minBindingSize = 1000u * sizeof(glm::mat4);
