...
StencilMask::StencilMask(StateMachine& machine) : State(machine, States::STENCIL_MASK) {
	...
	m_camera.lookAt(glm::vec3(0.0f, 0.0f, 35.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
void StencilMask::update() {
	...
	for (size_t i = 0; i < m_maskScenes.size(); i++) {
		updateSceneMask(glfwGetTime() * 5.0f, m_maskScenes[i], i, rotations[i]);
	}

	UpdateScene0(glfwGetTime(), m_scenes[0]);
	UpdateScene1(glfwGetTime(), m_scenes[1]);
	UpdateScene0(glfwGetTime(), m_scenes[2]);

	UpdateScene1(glfwGetTime(), m_scenes[3]);
	UpdateScene0(glfwGetTime(), m_scenes[4]);
	UpdateScene1(glfwGetTime(), m_scenes[5]);
	UpdateScene0(glfwGetTime(), m_scenes[6]);
}
...
void StencilMask::updateSceneMask(float time, Scene& scene, size_t index, float rotation[3]) {
	glm::mat4 view = glm::lookAt(glm::vec3(0.0f, 0.0f, 45.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	wgpuQueueWriteBuffer(wgpContext.queue, scene.sharedUniformBuffer.getBuffer(), offsetof(Uniforms, view), &view, sizeof(glm::mat4));
	...
	for (uint32_t i = 0; i < scene.numObjects; i++) {
		...
		wgpuQueueWriteBuffer(wgpContext.queue, scene.objects[i].uniformBuffer.getBuffer(), 0u, &transform.getTransformationMatrix(), sizeof(glm::mat4));
	}
}
...
void StencilMask::InitScene(Scene& scene, uint32_t numInstances, float hue, uint32_t geometryIndex, uint32_t geometryIndexCount) {
	...
	Uniforms uniforms;
	uniforms.projection = glm::perspective(glm::radians(30.0f), static_cast<float>(Application::Width) / static_cast<float>(Application::Height), 0.1f, 100.0f);
	uniforms.view = glm::lookAt(glm::vec3(0.0f, 0.0f, 35.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	uniforms.env = Camera::GetRotationMatrix(uniforms.view);
	uniforms.model = glm::mat4(1.0f);
	uniforms.normal = Camera::GetNormalMatrix(uniforms.view * uniforms.model);
	uniforms.color = { 0.0f, 1.0f, 0.4f, 1.0f };
	uniforms.camPosition = glm::vec3(0.0f, 0.0f, 35.0f);
	uniforms.lightVP = glm::mat4(1.0f);
	uniforms.shadow = glm::mat4(1.0f);
	uniforms.lightPosition = glm::normalize(glm::vec3(1.0f, 8.0f, 10.0f));
...
void StencilMask::UpdateScene0(float time, Scene& scene) {
	for (uint32_t i = 0; i < scene.numObjects; i++) {		
		Transform transform;
		transform.rotate(glm::degrees((time * 0.53f + i)), 0.0f, 0.0f, false);
		transform.translate(0.0f, 0.0f, sinf(i * 9.721f + time * 0.1f) * 10.0f);
		transform.rotate(0.0f, glm::degrees((i * 2.967f)), 0.0f, false);
		transform.rotate(glm::degrees((i * 4.567f)), 0.0f, 0.0f, false);
...
void StencilMask::UpdateScene1(float time, Scene& scene) {
	float radius = 35.0f;
	float t = time * 0.5f;
	glm::mat4 view = glm::lookAt(glm::vec3(cosf(t) * radius, 4.0f, sinf(t) * radius), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	wgpuQueueWriteBuffer(wgpContext.queue, scene.sharedUniformBuffer.getBuffer(), offsetof(Uniforms, view), &view, sizeof(glm::mat4));
