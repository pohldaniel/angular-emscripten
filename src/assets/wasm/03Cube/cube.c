...
void set_texture(const char* fileName) {
	/*int texture_width = 255;
	int texture_height = 255;
	int pitch = ((texture_width * 32 + 31) & ~31) >> 3; // align to 4-byte boundaries
	unsigned char* pixels = malloc( sizeof(unsigned char) * texture_height * pitch );
	for(int i = 0; i < sizeof(unsigned char) * texture_height * pitch; i = i + 4){
		pixels[i] = 255;
		pixels[i + 1] = 0;
		pixels[i + 2] = 0;
		pixels[i + 3] = 255;
