...
DeferredRendering::DeferredRendering(StateMachine& machine) : State(machine, States::DEFERRED_RENDERING) {
	...
	m_dragon.loadModel("res/models/dragon_vrip_res4.ply", glm::vec3(0.0f, 1.0f, 0.0f), 0.0f, glm::vec3(0.0f, -45.0f, 0.0f), 500.0f);
	...
	wgpContext.addSahderModule("DEFERRED", DEFERRED_WGSL, true);
	wgpContext.createRenderPipeline("DEFERRED", "RP_DEFERRED", VL_NONE, std::bind(&DeferredRendering::OnBindGroupLayoutsDeferred, this));

	wgpContext.addSahderModule("DEFERRED_DEBUG", DEFERRED_DEBUG_WGSL, true);
	...
	wgpContext.addSahderModule("GBUFFER", DEFERRED_GBUFFER_WGSL, true);
	...
	wgpContext.addSahderModule("COMPUTE", DEFERRED_COMPUTE_WGSL, true);
